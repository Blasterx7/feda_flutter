...
This is the most standard and secure way to use `feda_flutter` in production. You use a secure intermediary (like `ash-gate`) to handle requests.

```dart
FedaFlutter.applyCloudConfig(
  projectKey: 'your_public_project_key',
  cloudUrl: 'https://your-ash-gate.com',
);
```
