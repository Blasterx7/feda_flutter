...
class _PayWidgetState extends State<PayWidget> {
  ...
  void initiatePaymentFlow(TransactionCreate transaction, FedaFlutter feda) {
    ...
    _callCreateTransaction(feda).then((res) async {
      if (res == null) {
      ...
      if (body is Transaction) {
        try {
          ...
          paymentToken = body.paymentToken ?? json['payment_token']?.toString();
        } catch (_) {}
      }
  ...
  void initState() {
        ...
        NavigationDelegate(
          ...
          onNavigationRequest: (NavigationRequest request) {
            ...
            if (uri != null && uri.host.isNotEmpty) {
              ...
              if (!isAllowed) {
                debugPrint(
                    "Navigation blocked: domain not in whitelist: ${uri.host}");
    ...
    if (widget.transactionToken != null &&
        widget.transactionToken!.isNotEmpty) {
      final paymentUrl =
          'https://checkout.fedapay.com/pay/${widget.transactionToken}';
      _controller.loadRequest(Uri.parse(paymentUrl));
      return;
    }
    ...
    if (widget.instance == null) {
      throw Exception(
          'PayWidget requires either [transactionToken] or a configured [instance].');
    }
